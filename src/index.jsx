...
function App() {
  ...
  const [countdownStartSound] = useState(
    () => new Audio("./static/countdown-start.mp3")
  );
    ...
    if (phase === "ended") {
      ...
      setTimeout(() => {
        ...
        if (currentRecordTime > elapsedTime) {
          ...
          axios
            .patch(
              `https://circuit-rush-api.vercel.app/api/records/${userId}`,
              data,
              {
                headers: {
            ...
            .then(() => {
              ...
              setTimeout(() => {
                finishTime.current.innerHTML = `${formattedTime}<br><span class="better-time">(-${padWithZero(
                  Math.floor(timeDifference / 60000) % 60
                )}:${padWithZero(
                  Math.floor(timeDifference / 1000) % 60
                )}:${padWithZero(timeDifference % 1000, 3)})</span>`;
          ...
          if (!currentRecordTime) {
            ...
            setTimeout(() => {
              if (timeDifference == 0) {
                ...
                finishTime.current.innerHTML = `${formattedTime}<br><span class="worse-time">(+${padWithZero(
                  Math.floor(timeDifference / 60000) % 60
                )}:${padWithZero(
                  Math.floor(timeDifference / 1000) % 60
                )}:${padWithZero(timeDifference % 1000, 3)})</span>`;
  ...
  useEffect(() => {
    if (phase === "ended" && userId && userName && leaderboardView) {
      ...
      axios
        .get("https://circuit-rush-api.vercel.app/api/get-records")
        .then((response) => {
  ...
  const handleSubmit = () => {
    ...
    if (
      ...
      axios
        .post("https://circuit-rush-api.vercel.app/api/post-record", data, {
          headers: {
            "Content-Type": "application/json",
            "record-id": userId,
          },
    ...
    const countdown = async () => {
      for (let count = 3; count >= 1; count--) {
        ...
        gsap.fromTo(
          ...
          { opacity: 1, scale: 1.2 },
          { opacity: 0, scale: 1, duration: 0.6, delay: 0.4 }
        );
        ...
        <>
          ...
          <UserControls>
            ...
            >
              <Suspense fallback={<Loader />}>
                {!restartStatus ? (
                  <>
                    ...
                    <PerformanceMonitor
                      onIncline={() => setDpr(minDpr)}
                      onDecline={() => setDpr(maxDpr)}
                    >
                      <Experience
                        performanceMode={performanceMode}
                        gameBrightness={gameBrightness}
              ...
              {countdownStatus ? (
                <Html
                  wrapperClass={"countdown-overlay"}
                  ...
                  <>
                    {isExploding && (
                      <ConfettiExplosion
                        ...
                        colors={["#e55555", "#db3d3d", "#e55555", "#fc4c4c"]}
              ...
              {finishStatus && leaderboardView && userName ? (
                <Html
                  wrapperClass={"leaderboard-overlay"}
              ...
              (phase == "playing" || phase == "paused") ? (
                <Html
                  wrapperClass={"pause-button-overlay"}
